...
void Gemm_MRxNRKernel( int k, double *A, int ldA, double *B, int ldB,
		double *C, int ldC )
{
  /* Declare vector registers to hold 4x14 C and load them */
  ...
  __m256d gamma_0123_12 = _mm256_loadu_pd( &gamma( 0,12 ) );
  __m256d gamma_0123_13 = _mm256_loadu_pd( &gamma( 0,13 ) );
  ...
  for ( int p=0; p<k; p++ ){
    ...
    /* Load/broadcast beta( p,12 ). */
    beta_p_j = _mm256_broadcast_sd( &beta( p, 12) );
    
    /* update the twelfth column of C with the current column of A times
       beta ( p,12 ) */
    gamma_0123_12 = _mm256_fmadd_pd( alpha_0123_p, beta_p_j, gamma_0123_12 );

    /* Load/broadcast beta( p,13 ). */
    beta_p_j = _mm256_broadcast_sd( &beta( p, 13) );
    
    /* update the twelfth column of C with the current column of A times
       beta ( p,13 ) */
    gamma_0123_13 = _mm256_fmadd_pd( alpha_0123_p, beta_p_j, gamma_0123_13 );
  ...
  _mm256_storeu_pd( &gamma(0,12), gamma_0123_12 );
  _mm256_storeu_pd( &gamma(0,13), gamma_0123_13 );
